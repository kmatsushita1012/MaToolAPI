...
const postLocations = async (
  ...
  //変換
  const expirationTime = calculateExpirationTime(data.last_updated);
  const snakeData = toSnakeCase({ ...data, expirationTime: expirationTime });
  //属性の一致を確認
...
const calculateExpirationTime = (isoString: string) => {
  const date = new Date(isoString);
  const expirationDate = new Date(date.getTime() + 3 * 60 * 60 * 1000);
